...
- Migrated to MCP Python SDK 2.0 (`mcp>=2.0,<3`): `FastMCP` renamed to `MCPServer` in `server.py`; `pyproject.toml`/`uv.lock` updated accordingly. Tool decorator API, `mcp.run()`, and all 16 tool registrations are unaffected.
