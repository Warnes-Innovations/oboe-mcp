...
def test_oboe_create_returns_metadata(base_dir):
    result = oboe_create(
        base_dir=base_dir,
        title="My Session",
        description="desc",
...
def test_oboe_create_returns_completed_status_when_all_items_done(base_dir):
    result = oboe_create(
        base_dir=base_dir,
...
def test_oboe_create_updates_index(base_dir):
    oboe_create(
        base_dir=base_dir,
        title="Indexed",
        description="",
    ...
    from oboe_mcp.session import oboe_sessions_dir, load_index

    idx = load_index(oboe_sessions_dir(base_dir))
    assert any(
...
def test_oboe_create_duplicate_returns_error(base_dir, session_name):
    result = oboe_create(
        base_dir=base_dir,
        title="Dup",
        description="",
...
def test_oboe_create_rejects_invalid_filename(base_dir):
    result = oboe_create(
        base_dir=base_dir,
        title="Bad Filename",
        description="desc",
...
def test_oboe_create_without_items_creates_empty_session(base_dir):
    result = oboe_create(
        base_dir=base_dir,
        title="Empty Session",
...
def test_oboe_create_with_items_none_creates_empty_session(base_dir):
    result = oboe_create(
        base_dir=base_dir,
...
def test_oboe_list_sessions(base_dir, session_name):
    result = oboe_list_sessions(base_dir=base_dir)
    data = json.loads(result)
    assert data["total"] == 1
...
def test_oboe_list_sessions_empty(tmp_path):
    result = oboe_list_sessions(base_dir=str(tmp_path))
    data = json.loads(result)
...
def test_oboe_session_status(base_dir, session_name):
    result = oboe_session_status(session_file=session_name, base_dir=base_dir)
    data = json.loads(result)
    assert data["total"] == 3
...
def test_oboe_get_session(base_dir, session_name):
    result = oboe_get_session(session_file=session_name, base_dir=base_dir)
    data = json.loads(result)
...
def test_oboe_next_returns_highest_priority(base_dir, session_name):
    result = oboe_next(session_file=session_name, base_dir=base_dir)
    data = json.loads(result)
    assert data["title"] == "Alpha"


def test_oboe_next_no_items(base_dir):
    oboe_create(
        base_dir=base_dir,
        title="Empty",
        description="",
...
def test_oboe_next_returns_deferred_when_review_queue_is_exhausted(base_dir):
    oboe_create(
        base_dir=base_dir,
        title="Deferred",
        description="",
...
def test_oboe_list_items_sorted(base_dir, session_name):
    result = oboe_list_items(session_file=session_name, base_dir=base_dir)
    data = json.loads(result)
    scores = [i["priority_score"] for i in data["items"]]
    assert scores == sorted(scores, reverse=True)


def test_oboe_list_items_filter(base_dir, session_name):
    oboe_mark_complete(
        session_file=session_name,
        item_id="1",
        resolution="Done",
...
def test_oboe_get_item(base_dir, session_name):
    result = oboe_get_item(
        session_file=session_name,
        item_id="1",
        base_dir=base_dir,
    )
...
def test_oboe_get_item_not_found(base_dir, session_name):
    result = oboe_get_item(
        session_file=session_name,
        item_id="999",
        base_dir=base_dir,
...
def test_oboe_mark_complete(base_dir, session_name):
    result = oboe_mark_complete(
        session_file=session_name,
        item_id="1",
        resolution="Fixed",
        base_dir=base_dir,
...
def test_oboe_mark_complete_unknown_id(base_dir, session_name):
    result = oboe_mark_complete(
        session_file=session_name,
        item_id="999",
        resolution="?",
...
def test_oboe_mark_skip(base_dir, session_name):
    result = oboe_mark_skip(
        session_file=session_name,
        item_id="2",
...
def test_oboe_mark_skip_no_reason(base_dir, session_name):
    result = oboe_mark_skip(
        session_file=session_name,
        item_id="3",
        base_dir=base_dir,
...
def test_oboe_mark_deferred(base_dir, session_name):
    result = oboe_mark_deferred(
        session_file=session_name,
        item_id="2",
...
def test_oboe_mark_deferred_no_reason(base_dir, session_name):
    result = oboe_mark_deferred(
        session_file=session_name,
        item_id="3",
        base_dir=base_dir,
...
def test_oboe_mark_blocked(base_dir, session_name):
    result = oboe_mark_blocked(
        session_file=session_name,
        item_id="2",
...
def test_oboe_mark_in_progress(base_dir, session_name):
    result = oboe_mark_in_progress(
        session_file=session_name,
        item_id="2",
        base_dir=base_dir,
    )
...
def test_oboe_update_field_score_recalculates(base_dir, session_name):
...
def test_oboe_update_field_non_score(base_dir, session_name):
    result = oboe_update_field(
        session_file=session_name,
        item_id="1",
        field="title",
...
def test_oboe_update_field_approval_mode(base_dir, session_name):
    result = oboe_update_field(
        session_file=session_name,
        item_id="1",
        field="approval_mode",
...
def test_oboe_set_approval_defaults_to_immediate(base_dir, session_name):
    result = oboe_set_approval(
        session_file=session_name,
        item_id="1",
        approval_status="approved",
        base_dir=base_dir,
...
def test_oboe_set_approval_delayed_sets_deferred(base_dir, session_name):
    result = oboe_set_approval(
        session_file=session_name,
        item_id="1",
        approval_status="approved",
...
def test_oboe_set_approval_rejects_invalid_combination(base_dir, session_name):
    result = oboe_set_approval(
        session_file=session_name,
        item_id="1",
...
def test_oboe_update_field_rejects_invalid_approval_mode(
    base_dir,
    session_name,
):
    result = oboe_update_field(
...
def test_oboe_update_field_unknown_id(base_dir, session_name):
    result = oboe_update_field(
        session_file=session_name,
        item_id="999",
        field="title",
...
def test_oboe_update_field_rejects_invalid_status(base_dir, session_name):
    result = oboe_update_field(
        session_file=session_name,
        item_id="1",
        field="status",
...
def test_oboe_complete_session_rejects_actionable_items(base_dir, session_name):
    result = oboe_complete_session(session_file=session_name, base_dir=base_dir)
    assert result.startswith("ERROR:")


def test_oboe_complete_session(base_dir):
    result = oboe_create(
        base_dir=base_dir,
        title="Finished",
        description="done",
    ...
    complete_result = oboe_complete_session(
...
def test_oboe_merge_items(base_dir, session_name):
...
def test_oboe_create_child_session(base_dir, session_name):
...
def test_oboe_create_child_session_without_items(base_dir, session_name):
    result = oboe_create_child_session(
        parent_session_file=session_name,
        title="Empty Child",
...
def test_oboe_complete_child_session(base_dir, session_name):
    create_result = oboe_create_child_session(
        parent_session_file=session_name,
        title="Nested Investigation",
        description="child flow",
        items=[{"title": "Child item"}],
    ...
    complete_item_result = oboe_mark_complete(
    ...
    result = oboe_complete_child_session(
...
def test_oboe_complete_child_session_cancelled_disposition(base_dir, session_name):
    oboe_create_child_session(
        parent_session_file=session_name,
...
def test_oboe_complete_child_session_cancelled_allows_open_items(base_dir, session_name):
    oboe_create_child_session(
        parent_session_file=session_name,
...
def test_oboe_complete_child_session_invalid_disposition(base_dir, session_name):
    oboe_create_child_session(
        parent_session_file=session_name,
        title="X",
        description="",
    ...
    next_result = oboe_next(
        session_file=created_session_name,
        base_dir=base_dir,
    )
    assert json.loads(next_result)["id"] == "phase-1"

    in_progress_result = oboe_mark_in_progress(
        session_file=created_session_name,
        item_id="phase-1",
        base_dir=base_dir,
    )
    ...
    resumed_result = oboe_next(
        session_file=created_session_name,
        base_dir=base_dir,
    )
    assert json.loads(resumed_result)["id"] == "phase-1"

    merge_result = oboe_merge_items(
        session_file=created_session_name,
    ...
    blocked_result = oboe_mark_blocked(
        session_file=created_session_name,
        item_id="phase-2",
    ...
    child_result = oboe_create_child_session(
    ...
    child_item_complete = oboe_mark_complete(
        session_file=child_session_name,
        item_id="1",
    ...
    child_close_result = oboe_complete_child_session(
        child_session_file=child_session_name,
        base_dir=base_dir,
    ...
    assert oboe_complete_session(
        session_file=created_session_name,
        base_dir=base_dir,
    ).startswith("ERROR:")

    complete_result = oboe_mark_complete(
        session_file=created_session_name,
        item_id="phase-1",
    ...
    delayed_approval_result = oboe_set_approval(
        session_file=created_session_name,
        item_id="phase-2",
        approval_status="approved",
        approval_mode="delayed",
    ...
    final_complete_result = oboe_mark_complete(
        session_file=created_session_name,
        item_id="phase-3",
        resolution="Verified",
        base_dir=base_dir,
    ...
    deferred_next_result = oboe_next(
        session_file=created_session_name,
        base_dir=base_dir,
    )
    assert json.loads(deferred_next_result)["id"] == "phase-2"

    finish_deferred_result = oboe_mark_complete(
        session_file=created_session_name,
        item_id="phase-2",
    ...
    status_result = oboe_session_status(
        session_file=created_session_name,
        base_dir=base_dir,
    )
    ...
    close_result = oboe_complete_session(
        session_file=created_session_name,
        base_dir=base_dir,
    )
    ...
    sessions_result = oboe_list_sessions(
        base_dir=base_dir,
        status_filter="completed",
    )
...
def test_oboe_create_returns_error_for_nonexistent_base_dir():
    result = oboe_create(
...
def test_oboe_list_sessions_returns_error_for_nonexistent_base_dir():
    result = oboe_list_sessions(base_dir="/nonexistent/path/that/does/not/exist")
    assert result.startswith("ERROR:")
    assert "base_dir does not exist" in result
    assert "REMOTE host" in result


def test_oboe_session_status_returns_error_for_nonexistent_base_dir():
...
def test_oboe_next_returns_error_for_nonexistent_base_dir():
    result = oboe_next(
        session_file="session_20260314_120000.json",
        base_dir="/nonexistent/path/that/does/not/exist",
    )
...
def test_oboe_cancel_session(base_dir, session_name):
    result = json.loads(oboe_cancel_session(session_file=session_name, base_dir=base_dir))
    assert result["status"] == "ok"
    assert result["session_status"] == "cancelled"


def test_oboe_cancel_session_with_reason(base_dir, session_name):
    result = json.loads(oboe_cancel_session(session_file=session_name, base_dir=base_dir,
                                           cancel_reason="superseded"))
    assert result["cancel_reason"] == "superseded"


def test_oboe_cancel_session_reports_open_items(base_dir, session_name):
    result = json.loads(oboe_cancel_session(session_file=session_name, base_dir=base_dir))
    assert result["open_items_remaining"] == len(SAMPLE_ITEMS)


def test_oboe_cancel_session_missing_file(base_dir):
    result = oboe_cancel_session(session_file="session_20260101_000000.json", base_dir=base_dir)
    assert result.startswith("ERROR:")
...
def test_oboe_trim_sessions_completed(base_dir):
    from oboe_mcp.session import create_session
    sessions_dir = Path(base_dir) / ".github" / "oboe_sessions"
    ...
    result = json.loads(oboe_trim_sessions(base_dir=base_dir, before="now",
                                          status_filter="completed"))
    assert result["total_deleted"] >= 1
    assert not sf.exists()


def test_oboe_trim_sessions_dry_run(base_dir):
    from oboe_mcp.session import create_session
    sessions_dir = Path(base_dir) / ".github" / "oboe_sessions"
    sessions_dir.mkdir(parents=True, exist_ok=True)
    sf = sessions_dir / "session_20260101_010000.json"
...
def test_oboe_trim_sessions_any_status(base_dir):
    ...
    result = json.loads(oboe_trim_sessions(base_dir=base_dir, before="now", status_filter="any"))
